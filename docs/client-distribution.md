...
The Android job installs its pinned SDK, build tools, NDK, JDK, and Rust targets. It then runs `tauri android init` in the clean checkout, builds one universal release APK, aligns it for 16 KiB pages, signs it, verifies the signer, and checks all supported native ABIs. Nothing under the ignored `apps/client/src-tauri/gen/` directory is required from source control.
