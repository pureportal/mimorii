...
The desktop agent supports automatic service installation on Linux and Windows. Release automation ships the x64 targets already built and tested by CI. Windows artifacts are published unsigned. The Android APK is aligned for 16 KiB pages before it is signed and verified.
...
The release reuses the `CI` workflow as a required publish gate. After CI and every platform build succeed, it publishes the application and agent container images, creates `v<version>` at the exact bump commit, and creates the corresponding GitHub Release with generated notes and all nine files. Re-running the same workflow is idempotent: the tag must still identify the same commit and release assets are replaced by name.

Ordinary pushes continue through `CI`, including release metadata validation, tests, release builds, workflow linting, and both container validation jobs. They do not publish images, tags, workflow artifacts, or GitHub Releases.

## Container architecture

Mimorii publishes the application image as `ghcr.io/pureportal/mimorii` and the check-only agent image as `ghcr.io/pureportal/mimorii-agent`. The application image contains the Nest API and compiled web client. The agent image runs HTTP, TCP, and DNS checks without host telemetry.
...
The `mimorii` and `mimorii-agent` packages must grant this repository write access under **Package settings > Manage Actions access** if they are not already linked. Package visibility is independent of the public repository. Change each package to public manually only when anonymous pulls are intended; the workflow does not change package visibility.
...
The application container uses a multi-stage build and `pnpm deploy --prod` so the runtime contains only the API's production dependency closure, compiled API, and compiled web client. The agent container uses a multi-stage Rust build and runs as a non-root user. BuildKit caches dependency and image layers without placing cache contents in either runtime image.
...
docker build -t mimorii-agent:local apps/agent-desktop
```
