...
fn install_platform(executable: &Path) -> Result<()> {
    ...
    let unit = directory.join("mimorii-agent.service");
    let content = format!(
        "[Unit]\nDescription=Mimorii read-only monitoring agent\nAfter=network-online.target\nWants=network-online.target\n\n[Service]\nType=simple\nExecStart=\"{}\" run\nRestart=on-failure\nRestartSec=10\nNoNewPrivileges=true\nPrivateTmp=true\nProtectSystem=strict\nProtectHome=read-only\n\n[Install]\nWantedBy=default.target\n",
        executable.display()
    );
    ...
    command(
        "systemctl",
        &["--user", "enable", "--now", "mimorii-agent.service"],
    )?;
...
fn uninstall_platform() -> Result<()> {
    let _ = command(
        "systemctl",
        &["--user", "disable", "--now", "mimorii-agent.service"],
    );
    let directory = directories::BaseDirs::new()
        .ok_or_else(|| anyhow::anyhow!("could not determine the user home directory"))?
        .config_dir()
...
fn install_platform(executable: &Path) -> Result<()> {
    ...
    command("schtasks.exe", &["/Run", "/TN", "Mimorii Agent"])?;
    println!("installed Mimorii Agent startup task");
    Ok(())
}
...
#[cfg(windows)]
fn uninstall_platform() -> Result<()> {
    command("schtasks.exe", &["/Delete", "/F", "/TN", "Mimorii Agent"])
}
