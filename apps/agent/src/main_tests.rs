...
fn parses_all_runtime_and_service_commands() {
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "run"])
            .unwrap()
            .command,
        Command::Run
    ));
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "once"])
            .unwrap()
            .command,
        Command::Once
    ));
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "doctor"])
            .unwrap()
            .command,
        Command::Doctor
    ));
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "status"])
            .unwrap()
            .command,
        Command::Status
    ));
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "service", "install"])
            .unwrap()
            .command,
        Command::Service {
    ...
    assert!(matches!(
        Cli::try_parse_from(["mimorii-agent", "service", "uninstall"])
            .unwrap()
            .command,
        Command::Service {
...
fn rejects_missing_arguments_and_local_interval_configuration() {
    assert!(Cli::try_parse_from(["mimorii-agent"]).is_err());
    assert!(
        Cli::try_parse_from([
            "mimorii-agent",
            "enroll",
