...
fn tcp_check_reports_connection_and_configuration_failures() {
    let (listener, port) = tcp_listener();
    drop(listener);
    let connection = super::super::execute(
        &task(CheckType::Tcp, json!({ "host": "127.0.0.1", "port": port })),
        &snapshot(),
    );
    assert_result(
    ...
    let invalid = super::super::execute(&task(CheckType::Tcp, json!({})), &snapshot());
    assert_result(
        &invalid,
        CheckState::Down,
...
fn dns_check_rejects_invalid_configuration_and_record_types() {
    let malformed = super::super::execute(&task(CheckType::Dns, json!({})), &snapshot());
    assert_result(
        &malformed,
        CheckState::Down,
    ...
    let invalid_type = super::super::execute(
        &task(
            CheckType::Dns,
