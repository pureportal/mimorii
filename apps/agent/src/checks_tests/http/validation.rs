...
fn http_check_rejects_missing_and_invalid_expected_statuses() {
    let missing = crate::checks::execute(
...
fn http_check_validates_urls_credentials_and_methods() {
    ...
    assert!(crate::checks::http(&malformed).is_err());

    let protocol = http_task("file:///tmp/health", json!({}));
    assert_eq!(
        crate::checks::http(&protocol).unwrap_err().to_string(),
        "HTTP URL is invalid"
    );

    let credentials = http_task("http://user:password@localhost/health", json!({}));
    let credentials_result = crate::checks::execute(&credentials, &snapshot());
    assert_result(
        &credentials_result,
        CheckState::Down,
    ...
    let method_result = crate::checks::execute(&method, &snapshot());
    assert_result(
        &method_result,
        CheckState::Down,
...
fn http_check_reports_connection_failures_and_timeouts() {
    ...
    let connection = crate::checks::execute(
    ...
    let timeout = crate::checks::execute(&timeout_task, &snapshot());
