...
fn http_check_reports_unexpected_status_content_and_headers() {
    ...
    let status = crate::checks::execute(&http_task(&status_server.url, json!({})), &snapshot());
    assert_result(
        &status,
        CheckState::Down,
    ...
    let content = crate::checks::execute(
    ...
    let header = crate::checks::execute(
        &http_task(
            &header_server.url,
...
fn http_json_assertions_report_each_failure_path() {
    ...
    let invalid = crate::checks::execute(
    ...
    let mismatch = crate::checks::execute(
        &http_task(
            &mismatch_server.url,
...
fn http_redirect_failures_are_reported() {
    ...
    let loop_result = crate::checks::execute(
    ...
    let protocol = crate::checks::execute(
