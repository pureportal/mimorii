import { checkTypes, type CheckConfig, type CheckType } from "@mimorii/contracts";
...
describe("development seed", () => {
  it("targets the t4ggno development account and resets its credentials", async () => {
    ...
    const get = vi.fn(async (sql: string) => {
      ...
      if (sql.includes("FROM agents a JOIN resources")) return { id: "agent-local" };
      return undefined;
    });
  ...
  it("persists a comprehensive configuration in the selected account team", async () => {
    ...
    expect(results.length).toBeGreaterThanOrEqual(312);
    ...
    const agentChecks = checks.filter((call) => call.parameters[3] !== null);
    const directChecks = checks.filter((call) => call.parameters[3] === null);
    ...
    for (const check of checks) {
      expect(() =>
        validator.validate(
          check.parameters[5] as CheckType,
          JSON.parse(String(check.parameters[6])) as Record<string, unknown>
        )
      ).not.toThrow();
    }

    const simple = namedCheck(checks, "Website availability");
    expect(simple.parameters[3]).toBeNull();
    ...
    expect(health.parameters[3]).toBeNull();
    expect(status.parameters[3]).toBeNull();
    expect(checkConfig(health)).toMatchObject({
      jsonAssertions: {
        conditions: [{ pointer: "/status", expectedValue: "healthy" }],
      },
      expectedHeaders: { "content-type": "application/json" },
    });
    expect(checkConfig(status)).toMatchObject({
      jsonAssertions: {
        conditions: [{ pointer: "/page/status", expectedValue: "operational" }],
      },
    });

    const portChecks = checks.filter((call) => call.parameters[5] === "tcp");
    expect(portChecks.length).toBeGreaterThanOrEqual(8);
    expect(portChecks.some((call) => call.parameters[3] === null)).toBe(true);
    expect(portChecks.some((call) => call.parameters[3] !== null)).toBe(true);
...
function namedCheck(checks: RunCall[], name: string): RunCall {
  return checks.find((call) => call.parameters[4] === name)!;
}

function checkConfig(check: RunCall): CheckConfig {
  return JSON.parse(String(check.parameters[6])) as CheckConfig;
}
